...
"""Глобальная очередь для размещения ставок."""

# Очередь для последовательной проверки результатов сделок. Сервер отдаёт
# результат не всегда мгновенно, а в пиковые моменты может ограничивать
# частоту запросов. Лёгкая задержка в 0.5 c между запросами снижает риск
# отказов, не блокируя остальной трафик (получение payout, выставление
# новых сделок и т.п.).
result_check_queue = TradeQueue(trade_delay_seconds=0.5)
